 ...
 * @author bilga
 */
@Name( "debtorContactReport" )
@Scope(ScopeType.SESSION)
public class DebtorContactReportBean {
    ...
    private List<ContactReportModel> debtorContacts;
    private List<ContactReportSummaryModel> debtorContactSummary;
    ...
    public void search(){
    	initDebtorContacts();
    	initDebtorContactSummary();
    }
 
    private void initDebtorContacts() {
    	
    	if(debtorContacts != null){
    		debtorContacts.clear();
		}
		else{
			debtorContacts = new ArrayList<ContactReportModel>();
		}
    	
    	List<Object []> debtor = getDebtorContact();
    	
    	for (Object[] item : debtor){
			...
			debtorContacts.add(model);
		}
//    	debtorContacts.addAll(getDebtorContact());
	}

    private void initDebtorContactSummary() {
    	
    	if(debtorContactSummary != null){
    		debtorContactSummary.clear();
		}
		else{
			debtorContactSummary = new ArrayList<ContactReportSummaryModel>();
		}
    	
    	List<Object []> debtor = getDebtorContactsSummary();
    	...
    	for (Object[] item : debtor){
			...
			debtorContactSummary.add(model);
		}
    	
    	
//    	debtorContactSummary.addAll(getDebtorContactsSummary());
    }

    public BigDecimal getGrandTotal() {
    	BigDecimal total = BigDecimal.ZERO;
    	for (ContactReportSummaryModel model : debtorContactSummary) {
	...
	public List<Object []> getDebtorContact(){
    ...
    @SuppressWarnings("unchecked")
    public List<Object []> getDebtorContactsSummary(){
	...
	public void pdf(){
        ...
        try {
            jasperReport.reportToPDF( "debtorContactList", "Borclu_Cari_Raporu", params);
	...
	public void xls(){
        ...
        try {
        	 jasperReport.reportToXls( "debtorContactList_xls", "Borclu_Cari_Raporu", params);
        } catch (JRException ex) {
            log.error("Rapor Hatası", ex);
            facesMessages.add("Rapor Çalıştırılamadı");
        }
    ...
    public boolean renderContact( int ix ){
        if( debtorContacts == null ) return false;
        if( ix == 0 ) return true;
        String curval = debtorContacts.get(ix).getContactCode();
        String oldval = debtorContacts.get(ix - 1 ).getContactName();
        return !curval.equals( oldval );
    }

	public void setDebtorContacts(List<ContactReportModel> debtorContacts) {
		this.debtorContacts = debtorContacts;
	}

	public List<ContactReportModel> getDebtorContacts() {
		if (debtorContacts == null) {
			debtorContacts = new ArrayList<ContactReportModel>();
		}
	...
	public void setDebtorContactSummary(List<ContactReportSummaryModel> debtorContactSummary) {
		this.debtorContactSummary = debtorContactSummary;
	}

	public List<ContactReportSummaryModel> getDebtorContactSummary() {
		if (debtorContactSummary == null) {
			debtorContactSummary = new ArrayList<ContactReportSummaryModel>();
		}
		return debtorContactSummary;
	}
