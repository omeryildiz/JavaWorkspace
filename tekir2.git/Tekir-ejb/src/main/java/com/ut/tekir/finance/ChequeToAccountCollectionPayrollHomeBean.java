...
/**
 * Kasa çek tahsilat bordrosu home bileşenidir.
 * 
 * Açıklama:
 *     Müşteriden alınan çeklerin topluca girilebileceği bordrodur.
 *     Alınan çekler kasa tahsilat statüsüne girerler.
 *     Kasa hareket tablosuna kasa için borç yazar.
 *     Çek tarihçesi tablosunda hareket ile ilgili kayıt atılır.
 * 
 * @author sinan.yumak
 */
@Name("chequeToAccountCollectionPayrollHome")
@Scope(value = ScopeType.CONVERSATION)
public class ChequeToAccountCollectionPayrollHomeBean extends EntityBase<ChequeToAccountCollectionPayroll> implements ChequeToAccountCollectionPayrollHome<ChequeToAccountCollectionPayroll> {
    ...
    @Out(required = false)
	public ChequeToAccountCollectionPayroll getChequeToAccountCollectionPayroll(){
		return getEntity();
	}
	
	@In(required = false)
	public void setChequeToAccountCollectionPayroll(ChequeToAccountCollectionPayroll ChequeToAccountCollectionPayroll){
		setEntity(ChequeToAccountCollectionPayroll);
	}

	@Override
    public void createNew() {
        log.debug("Yeni ChequeToAccountCollectionPayroll");

        entity = new ChequeToAccountCollectionPayroll();
        entity.setSerial(sequenceManager.getNewSerialNumber(SequenceType.SERIAL_CHEQUE_TO_ACCOUNT_COLLECTION_PAYROLL));
	...
	public String save() {
		...
		try {
			...
			if(res.equals(BaseConsts.SUCCESS)){
				...
				for(ChequeToAccountCollectionPayrollDetail cpd : entity.getDetails()){
					
					Cheque cheque = cpd.getCheque();
	    			
					if (! chequeAction.alreadySaved(cheque, DocumentType.ChequeAccountCollectionPayroll, entity.getId())) {
						...
						cpm.setDocumentType(DocumentType.ChequeAccountCollectionPayroll);
				...
				accountTxnAction.saveChequeToAccountCollectionPayroll(entity);
    ...
    public void createNewLine() {
        ...
        ChequeToAccountCollectionPayrollDetail detail = new ChequeToAccountCollectionPayrollDetail();
        detail.setOwner(entity);
        entity.getDetails().add(detail);
        log.debug("yeni detay eklendi");
    }
    
    public void deleteLine(ChequeToAccountCollectionPayrollDetail detail) {
        manualFlush();
        if (entity == null) {
            return;
        }
    ...
    public void popupChequeSelect(int clientOrFirm) {
		chequeSuggestion.setCallerObserveString("ChequeToAccountCollectionPayrollHomeBean:popupNotice:client.cheque");
		chequeSuggestion.setClientCheque(Boolean.TRUE);
		chequeSuggestion.setTargetStatus(ChequeStatus.KasaTahsilat);
		
    }
  
	@Observer("ChequeToAccountCollectionPayrollHomeBean:popupNotice:client.cheque")
	public void selectClientCheque(Cheque cheque) {
		...
		if (entity == null) {
	 	...
	 	ChequeToAccountCollectionPayrollDetail cpd = new ChequeToAccountCollectionPayrollDetail();
	 	cpd.setOwner(entity);
    ...
    public String delete() {
        accountTxnAction.deleteChequeToAccountCollectionPayroll(entity);
		...
		try {
			...
			if(entity.getId() != null){
				...
				if( entity.getDetails() != null && entity.getDetails().size() > 0){
					...
					for(ChequeToAccountCollectionPayrollDetail items : getEntity().getDetails()) {

						if(items.getId() == null){
							return;
						}
						Cheque cheque = items.getCheque();
						if(cheque.getLastStatus() != ChequeStatus.KasaTahsilat){
	...
	@Override
	public void setEntity(ChequeToAccountCollectionPayroll entity) {
